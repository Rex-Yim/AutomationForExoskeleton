...
% Figure path: project results folders. Build: docs/latex/compile.sh → docs/reports/final_report.pdf
...
\begin{itemize}
  \item \textbf{Data acquisition and processing:} \textbf{HuGaDB} deployment path; held-out HuGaDB replay for pseudo-real-time testing (including \textbf{multiclass LSTM} replay on subject~\ReplayHuGaDBSubject{}, session~\ReplayHuGaDBSession{} as the emphasized sequence-model visualization); optional Kalman visualization (\texttt{imufilter}); \textbf{binary} RBF SVM on HuGaDB 30-D window features with USC-HAD / HuGaDB benchmark comparisons retained for the paper and poster; optional \textbf{native ECOC} SVMs (twelve USC-HAD / twelve HuGaDB activity classes); optional \textbf{binary and multiclass LSTM} sequence models on per-dataset IMU windows (\texttt{TrainLstmBinary}, \texttt{TrainLstmMulticlass}, \texttt{RunExoskeletonPipelineLstm}). Embedded deployment remains future work.
...
% ---------- Appendices (external PDFs; paths relative to docs/latex/) ----------
\appendix
\chapter{HuGaDB GitHub v1 Corpora}
\includepdf[pages=-]{../reports/hugadb_github_v1_corpora.pdf}

\chapter{Pipeline Gallery (Full)}
\includepdf[pages=-]{../reports/pipeline_gallery_full.pdf}

\chapter{Poster}
\includepdf[pages=-]{../reports/poster.pdf}

\chapter{System Design}
\includepdf[pages=-]{../reports/System_Design.pdf}
