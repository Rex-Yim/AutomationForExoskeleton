...
\begin{itemize}[noitemsep]
  \item \textbf{Data Acquisition and Processing Pipeline:} Three-phone CSV prototype; Kalman fusion (\texttt{imufilter}); \textbf{binary} RBF SVM training on merged USC-HAD and HuGaDB (30-D window features) with USC-only / HuGaDB-only ablations; optional \textbf{twelve-class} ECOC SVM. LSTM and embedded deployment remain future work.
  \item \textbf{Proactive Real-Time Locomotion Intent Recognition:} Binary task = \textbf{locomotion-like} vs.\ \textbf{non-locomotion} from dataset label maps. The $>90$\% target is met for USC-HAD-only and merged binary training; HuGaDB-only binary accuracy is lower (imbalance and label noise). The 100--200~ms horizon is a design target, not re-benchmarked here for the SVM-only baseline.
\end{itemize}
